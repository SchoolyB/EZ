...
func TestResolveTemplate(t *testing.T) {
	cases := []struct {
		...
		{"basic", "", "templates/basic", "main.ez"},
		{"cli", "", "templates/cli", "main.ez"},
		{"lib", "", "templates/lib", "main.ez"},
		{"multi", "", "templates/multi", "main.ez"},
		{"server", "minimal", "templates/server_minimal", "main.ez"},
		{"server", "normal", "templates/server_normal", "main.ez"},
		{"client", "minimal", "templates/client_minimal", "main.ez"},
		{"client", "normal", "templates/client_normal", "main.ez"},
	}
	for _, c := range cases {
...
func TestCreateProject_WithComments(t *testing.T) {
	...
	main := filepath.Join(name, "main.ez")
	data, err := os.ReadFile(main)
	if err != nil {
		t.Fatalf("main.ez not created: %v", err)
	}
	if len(data) == 0 {
		t.Fatal("main.ez is empty")
	}
