...
func TestHasMainFunction(t *testing.T) {
	...
	withMain := filepath.Join(dir, "with_main.ez")
	os.WriteFile(withMain, []byte("do main() {\n    println(\"hi\")\n}\n"), 0o644)

	withMainArgs := filepath.Join(dir, "with_main_args.ez")
	os.WriteFile(withMainArgs, []byte("do main(args [string]) {\n}\n"), 0o644)

	noMain := filepath.Join(dir, "no_main.ez")
	os.WriteFile(noMain, []byte("do helper() {\n}\n"), 0o644)

	empty := filepath.Join(dir, "empty.ez")
	...
	cases := []struct {
		...
		{"nonexistent", filepath.Join(dir, "nope.ez"), false},
	}
	for _, c := range cases {
...
func TestScanImports_NonexistentFile(t *testing.T) {
	got := scanImports("/no/such/file.ez")
	if got != nil {
...
func TestCollectEzFilesInDir(t *testing.T) {
	...
	os.WriteFile(filepath.Join(dir, "a.ez"), []byte(""), 0o644)
	os.WriteFile(filepath.Join(dir, "b.ez"), []byte(""), 0o644)
	os.WriteFile(filepath.Join(dir, "notes.txt"), []byte(""), 0o644)
	os.WriteFile(filepath.Join(sub, "c.ez"), []byte(""), 0o644)
...
func TestFindMainFile(t *testing.T) {
	...
	t.Run("no main function", func(t *testing.T) {
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, "lib.ez"), []byte("do helper() {}\n"), 0o644)
		_, err := findMainFile(dir)
		if err == nil {
	...
	t.Run("single main file", func(t *testing.T) {
		dir := t.TempDir()
		main := filepath.Join(dir, "main.ez")
	...
	t.Run("multiple main files", func(t *testing.T) {
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, "a.ez"), []byte("do main() {}\n"), 0o644)
		os.WriteFile(filepath.Join(dir, "b.ez"), []byte("do main() {}\n"), 0o644)
...
func TestCollectFilesToWatch_StdlibSkipped(t *testing.T) {
	...
	// Only main.ez — stdlib imports must not be resolved to disk paths
	if len(files) != 1 {
...
func TestCollectFilesToWatch_LocalImport(t *testing.T) {
	dir := t.TempDir()
	main := filepath.Join(dir, "main.ez")
	util := filepath.Join(dir, "util.ez")
	...
	if names[0] != "main.ez" || names[1] != "util.ez" {
		t.Errorf("got %v, want [main.ez util.ez]", names)
	}
}
...
func TestShortPath(t *testing.T) {
	...
	abs := filepath.Join(cwd, "some", "file.ez")
	got := shortPath(abs)
	...
	outside := "/completely/different/path/program.ez"
