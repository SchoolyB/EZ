/*
 * ez_crypto.c - @crypto module implementation
 *
...
EzString ez_crypto_sha256(EzArena *arena, EzString data) {
    ...
    uint8_t *msg = (uint8_t *)ez_arena_alloc(arena, padded);
    ...
    char *hex = ez_arena_alloc(arena, 65);
    for (int i = 0; i < 8; i++)
...
EzString ez_crypto_md5(EzArena *arena, EzString data) {
    ...
    char *hex = ez_arena_alloc(arena, 33);
    uint8_t digest[16];
