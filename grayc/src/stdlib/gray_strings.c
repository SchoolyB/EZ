/*
 * gray_strings.c - @strings module implementation
 *
 * Copyright (c) 2025-Present Marshall A Burns
 * Licensed under the MIT License. See LICENSE for details.
 */

#include "gray_strings.h"
#include <ctype.h>
#include <stdlib.h>
#include <string.h>
...
EzArray gray_strings_split(EzArena *arena, EzString s, EzString sep) {
    ...
    for (int32_t i = 0; i <= s.len - sep.len; i++) {
        if (memcmp(s.data + i, sep.data, (size_t)sep.len) == 0) {
            EzString part = gray_string_new(arena, s.data + start, i - start);
            gray_array_push(arena, &arr, &part);
            i += sep.len - 1;
            start = i + 1;
        }
    }
    EzString last = gray_string_new(arena, s.data + start, s.len - start);
...
EzArray gray_strings_to_chars(EzArena *arena, EzString s) {
    EzArray arr = gray_array_new(arena, sizeof(int32_t), s.len);
