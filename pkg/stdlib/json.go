...
// formatJSONValue formats a single value for pretty printing
func formatJSONValue(obj object.Object, indent string, currentIndent string, seen map[uintptr]bool) (string, *jsonError) {
	switch v := obj.(type) {
	case *object.Integer:
		// Check if value fits in int64, otherwise encode as string to preserve precision
		if v.Value.IsInt64() {
	...
	case *object.Char:
		bytes, _ := json.Marshal(string(v.Value))
		return string(bytes), nil
	...
	case *object.Array:
		...
		for i, elem := range v.Elements {
			...
			val, err := formatJSONValue(elem, indent, newIndent, seen)
			if err != nil {
				return "", err
			}
			sb.WriteString(val)
	...
	case *object.Map:
		...
		for _, pair := range v.Pairs {
			...
			items = append(items, keyValue{key: keyStr.Value, val: pair.Value})
		}
		...
		for i, item := range items {
			...
			sb.WriteString(string(keyBytes))
			sb.WriteString(": ")
			val, err := formatJSONValue(item.val, indent, newIndent, seen)
			if err != nil {
				return "", err
			}
			sb.WriteString(val)
	...
	case *object.Struct:
		...
		for i, key := range keys {
			...
			sb.WriteString(string(keyBytes))
			sb.WriteString(": ")
			val, err := formatJSONValue(v.Fields[key], indent, newIndent, seen)
			if err != nil {
				return "", err
			}
			sb.WriteString(val)
